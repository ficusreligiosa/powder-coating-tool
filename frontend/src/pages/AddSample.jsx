...
export default function AddSample() {
  ...
  useEffect(() => {
    if (citySearch.length < 1) { setCityResults([]); return }
    const timer = setTimeout(async () => {
      try {
  ...
  const addProduct = () => {
    setForm(f => ({ ...f, products: [...f.products, { product_name: '', quantity: '', matched_with: '' }] }))
  }
  ...
  const updateProduct = (index, field, value) => {
    setForm(f => ({ ...f, products: f.products.map((p, i) => i === index ? { ...p, [field]: value } : p) }))
  }
    ...
    <Layout>
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 md:p-6 space-y-5">
          ...
          <div>
                ...
                <div className="absolute z-10 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden max-h-52 overflow-y-auto">
                  {partyResults.map(party => (
                    <button
                      ...
                      className="w-full text-left px-4 py-3.5 hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0"
                    >
                  ...
                  <button
                    ...
                    className="w-full text-left px-4 py-3.5 hover:bg-violet-50 text-violet-600 text-sm font-medium"
                  >
            ...
            {selectedParty && (
              <div className="mt-2 flex items-center gap-2 flex-wrap">
                ...
                <button
                  onClick={() => { setSelectedParty(null); setPartySearch(''); setForm(f => ({ ...f, party_id: '' })) }}
            ...
            {showAddParty && (
              <div className="mt-3 p-4 bg-violet-50 rounded-xl border border-violet-100">
                <p className="text-xs font-semibold text-violet-700 mb-3 uppercase tracking-wider">Add New Party</p>
                <div className="space-y-2">
                  <input
                    ...
                    className="w-full px-3 py-3 border border-violet-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 bg-white"
                  />
                  <input
                    type="text"
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="flex-1 py-2.5 rounded-lg text-slate-600 text-sm border border-slate-200 bg-white"
                    >
                      Cancel
                    </button>
                  </div>
          ...
          {/* Remark */}
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">Remark (Optional)</label>
            <input
              type="text"
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                onClick={addProduct}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-violet-50 text-violet-600 rounded-lg text-xs font-medium hover:bg-violet-100"
              >
                ...
                <div key={index} className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {form.products.length > 1 && (
                      <button onClick={() => removeProduct(index)} className="text-xs text-red-400 hover:text-red-600">
                        Remove
                      </button>
                    )}
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <input
                      ...
                      className="w-full px-3 py-3 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 bg-white"
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              ...
              className="px-6 py-3 rounded-xl text-slate-600 text-sm font-medium border border-slate-200 hover:bg-slate-50"
            >
              Cancel
            </button>
          </div>
